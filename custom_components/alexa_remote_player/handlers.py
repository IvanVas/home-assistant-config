...
@HANDLERS.register(("Alexa.RemoteVideoPlayer", "SearchAndPlay"))
async def async_api_remote_video_player_search_and_play(
    hass, config, directive, context
):
    entity = directive.entity

    entities = directive.payload["entities"]

    if len(entities) > 0:
        video_entity = entities[0]
        imdb_id = video_entity["externalIds"]["imdb"]

        data = {
            ATTR_ENTITY_ID: entity.entity_id,
            media_player.const.ATTR_MEDIA_CONTENT_ID: imdb_id,
            media_player.const.ATTR_MEDIA_CONTENT_TYPE: "imdb",
        }
        ...
        msg = f"failed to find {directive.payload['searchText']['transcribed']}"
        raise AlexaInvalidValueError(msg)

    return directive.response()


@HANDLERS.register(("Alexa.RemoteVideoPlayer", "SearchAndDisplayResults"))
async def async_api_remote_video_player_search_and_display(
    hass, config, directive, context
):
    return directive.response()


@HANDLERS.register(("Alexa.Launcher", "LaunchTarget"))
async def async_api_launcher(hass, config, directive, context):
    """Process a set launch request."""
    app = directive.payload["name"]
    entity = directive.entity
    ...
    for source in source_list:
        ...
        if formatted_source == app:
            app = source
            break
    else:
        msg = f"failed to map input {app} to a media source on {entity.entity_id}"
        raise AlexaInvalidValueError(msg)

    data = {
        ATTR_ENTITY_ID: entity.entity_id,
        media_player.const.ATTR_INPUT_SOURCE: app,
    }
